...
import { confirmOracleCancellation, OracleQueueConflictError } from '@/lib/arxcian/oracleQueue'
import { createRedisOracleBackend } from '@/lib/arxcian/oracleRedisBackend'

type Body = { id?: unknown; claimToken?: unknown }
...
export async function POST(req: NextRequest) {
  ...
  if (typeof body.id !== 'string' || typeof body.claimToken !== 'string') {
    return NextResponse.json({ error: 'Virheellinen keskeytyskuittaus' }, { status: 400 })
  }
  try {
    const message = await confirmOracleCancellation(
      createRedisOracleBackend(kv()), body.id, body.claimToken,
    )
