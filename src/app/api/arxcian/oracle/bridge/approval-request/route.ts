...
type Body = { id?: unknown; claimToken?: unknown; approval?: unknown }
...
export async function POST(req: NextRequest) {
  ...
  const approval = body.approval as Partial<OracleApproval> | null
  if (typeof body.id !== 'string' || typeof body.claimToken !== 'string' || !approval
    || typeof approval.requestId !== 'string'
    || ![approval.command, approval.description, approval.tool].every(value => value === null || typeof value === 'string')) {
    return NextResponse.json({ error: 'Virheellinen hyväksyntäpyyntö' }, { status: 400 })
  }
  try {
    const message = await recordOracleApprovalRequest(
      createRedisOracleBackend(kv()), body.id, body.claimToken, approval as OracleApproval,
    )
