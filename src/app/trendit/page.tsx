...
interface MonthData {
  ...
  tyokulu: number
  erotusPct: number
  stores: Record<string, number> // liittymät € per myymälä
}
...
export default function TrendPage() {
  ...
  useEffect(() => {
    fetch('/api/receipts')
      .then(r => r.json())
      .then(async d => {
        if (d.error && !d.files?.length) { setError(d.error); setLoading(false); return }
        const files = ((d.files ?? []) as DriveFile[]).sort((a, b) => monthNumOf(a.name) - monthNumOf(b.name))

        const results: MonthData[] = []
        const allStoreNames = new Set<string>()
        for (const f of files) {
          const res = await fetch(`/api/receipts?fileId=${f.id}`)
          const data = await res.json()
          if (data.error || !data.totals) continue

          const netto = Math.round(data.totals.netto)
          const tyokulu = Math.round((data.sellers ?? []).reduce((s: number, r: { sivukulut: number }) => s + r.sivukulut, 0))
          const stores: Record<string, number> = {}
          for (const [nimi, s] of Object.entries(data.stores ?? {}) as [string, { liittymat: number }][]) {
            stores[nimi] = Math.round(s.liittymat)
            allStoreNames.add(nimi)
          }

          results.push({
            kuukausi: (data.kuukausi ?? f.name).replace(/^Maksukuitti\s+/i, ''),
            netto,
            provisio: Math.round(data.totals.provisio),
            liittEur: Math.round(data.totals.liittymat),
            fsecKpl: Math.round(data.totals.fsecAsiakkuudet),
            fsecPassiivi: Math.round(data.totals.fsecPassiivitulo),
            kassakate: Math.round(data.totals.kassakate),
            tyokulu,
        ...
        setMonths(results)
        setStoreNames(Array.from(allStoreNames))
        setLoading(false)
      })
  ...
  const liittTrendCoef = linearTrend(months.map(m=>m.liittEur))
  ...
  const storeTrendData = months.map(m => ({ kuukausi: m.kuukausi, ...m.stores }))
      ...
      <div style={{maxWidth:900,margin:'0 auto',padding:'16px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:8,marginBottom:16}}>
          {[
            ...
            {l:'Keskiarvo/kk', v: fmt(avgNetto), s:'netto (todellinen)'},
        ...
        <div style={{background:'white',border:'0.5px solid #eee',borderRadius:12,padding:'16px',marginBottom:12}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
            <div style={{fontSize:11,fontWeight:500,color:'#888',textTransform:'uppercase',letterSpacing:'0.5px'}}>Netto vs Työkulu (todellinen)</div>
          ...
          <div style={{background:'white',border:'0.5px solid #eee',borderRadius:12,padding:'16px'}}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={monthsWithTrend}>
                ...
                <Tooltip formatter={(v:number) => fmt(v)} />
                <Legend wrapperStyle={{fontSize:11}} />
                <Line type="monotone" dataKey="liittEur" name="Liittymät €" stroke="#185FA5" strokeWidth={2} dot={{r:4}} />
        ...
        <div style={{background:'white',border:'0.5px solid #eee',borderRadius:12,padding:'16px',marginBottom:12}}>
          <div style={{fontSize:11,fontWeight:500,color:'#888',textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:12}}>Myymäläkohtainen trendi — Liittymät €</div>
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={storeTrendData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
              <XAxis dataKey="kuukausi" tick={{fontSize:11}} />
              <YAxis tick={{fontSize:11}} />
              <Tooltip formatter={(v:number) => fmt(v)} />
              <Legend wrapperStyle={{fontSize:11}} />
              {storeNames.map(nimi => (
                <Line key={nimi} type="monotone" dataKey={nimi} name={nimi} stroke={STORE_COLORS[nimi] ?? '#888'} strokeWidth={2} dot={{r:3}} />
              ))}
            </LineChart>
          </ResponsiveContainer>
        </div>
        <div style={{background:'white',border:'0.5px solid #eee',borderRadius:12,overflow:'hidden'}}>
          <div style={{padding:'10px 14px',borderBottom:'0.5px solid #eee'}}>
            <span style={{fontWeight:500,fontSize:14}}>Kuukausipalkki (todellinen)</span>
          </div>
          <div style={{overflowX:'auto'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
            <thead>
              <tr style={{borderBottom:'0.5px solid #eee'}}>
                {['Kuukausi','Liittymät €','Provisio','F-Sec kpl','F-Sec passiivitulo','Kassakate','Työkulu','Netto'].map(h=>(
            ...
            <tbody>
              {months.map(m=>(
                <tr key={m.kuukausi} style={{borderBottom:'0.5px solid #f5f5f5'}}>
                  ...
                  <td style={{padding:'7px 12px',textAlign:'right'}}>{fmt(m.provisio)}</td>
                  ...
                  <td style={{padding:'7px 12px',textAlign:'right'}}>{fmt(m.kassakate)}</td>
              ...
              <tr style={{background:'#f8f8f6',borderTop:'1px solid #eee'}}>
                ...
                <td style={{padding:'7px 12px',textAlign:'right',fontWeight:600}}>{fmt(months.reduce((s,m)=>s+m.provisio,0))}</td>
                ...
                <td style={{padding:'7px 12px',textAlign:'right',fontWeight:600}}>{fmt(months.reduce((s,m)=>s+m.kassakate,0))}</td>
