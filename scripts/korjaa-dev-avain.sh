...
# Varmistus: luetaan takaisin samalla tavalla kuin sovellus lukee.
node -e '
  const fs = require("fs")
  const rivit = fs.readFileSync(".env.local", "utf8").split(/\r?\n/)
  const rivi = rivit.find(r => r.startsWith("GOOGLE_SERVICE_ACCOUNT_KEY="))
  if (!rivi) { console.error("VARMISTUS EPÄONNISTUI: muuttujaa ei ole .env.localissa"); process.exit(1) }
  let v = rivi.slice("GOOGLE_SERVICE_ACCOUNT_KEY=".length).trim()
