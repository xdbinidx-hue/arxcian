...
Käynnistä olemassa oleva loopback-preview pysyvämmin työkalukutsujen yli: `python3 -B ops/acceptance/start-preview.py`. Se ei ole boot-palvelu/valvonta, ei rakenna sovellusta eikä muuta tuotantoa. Kieltää jo käytössä olevat portit. Proxy hyväksyy localhostin porttivälityksen vaihtelevan portin, vieras Host edelleen 403.

Winpos-erittely luetaan nyt readonly suoraan arkistosta. Nykyinen capture lukee myös valitun uusimman raportin. Tiedostonimen päivästä vähennetään yksi päivä käyttäjän vahvistaman päättymispäivän saamiseksi; jaksonAlku null, ei väitetä tuntematonta aikaväliä koko kuukaudeksi. Ei raporttien yhteenlaskua, Sheets-tuontia tai cron-aktivointia. Cash-report metadata kuuluu Oracle-fingerprintiin ja DOM-check vaatii päättymispäivän näkymisen sivulla.

Previewn Drive-lukutilanne päivittyy nyt erillisellä `capture-drive-view.cjs --watch` -prosessilla 60 sekunnin välein. start-preview.py käynnistää sen private drive-candidate.json:n löytyessä. Saman watcherin lukuja ei ajeta päällekkäin. Lukijalla vain enforced readonly-scope; ei app-avaimen aktivointia tai malliajoa. Capturen rename on atominen, epäonnistunut/puutteellinen luku ei korvaa viimeistä onnistunutta. Proxy lukee uuden kokonaisen capturen pyynnön alussa; selain päivittää kolme lukureittiä näkyvänä 60s välein ja focus-tapahtumasta. Tietojen latausraja säilyy yhteisenä.
