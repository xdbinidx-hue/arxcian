 ...
 for(const name of ['tavoitteet','uusmyynti','kassamyynti']){
  ...
  if(name==='tavoitteet'){
    ...
    for(const seller of snapshot.dash.sellers){
      ...
      if(seller.tyyppi!=='owner' && seller.tunnit>0 && Number.isFinite(seller.myyntiTeho)){const expected=seller.myyntiTeho>=9?'rgb(220, 252, 231)':seller.myyntiTeho>=8?'rgb(254, 249, 195)':seller.myyntiTeho>=7?'rgb(255, 237, 213)':'rgb(254, 226, 226)';assert.equal(row.children[9].style.background,expected,'Combined efficiency cell must have colored background')}
    }
  }
